...
#ifndef NDEBUG
    for (int32_t i=0; i<_2N; i++) assert(fabs(imag_inout[i]) < 0.1);
    for (int32_t i=0; i<N; i++) assert(fabs(real_inout[i]+real_inout[N+i]) < 0.1);
#endif
}

void FFT_Processor_nayuki::check_conjugate_cplx() {
#ifndef NDEBUG
    for (int32_t i=0; i<N; i++) assert(fabs(real_inout[2*i])+fabs(imag_inout[2*i]) < 0.1);
    for (int32_t i=0; i<Ns2; i++) assert(fabs(imag_inout[2*i+1]+imag_inout[_2N-1-2*i]) < 0.1);
#endif
}
...
void FFT_Processor_nayuki::execute_reverse_int(cplx* res, const int32_t* a) {
    ...
    {
        assert(abs(cplx(real_inout[2*i+1],imag_inout[2*i+1])-res[i]) < 0.1);
    }
...
thread_local FFT_Processor_nayuki  fp512_nayuki(512);
thread_local FFT_Processor_nayuki fp1024_nayuki(1024);
thread_local FFT_Processor_nayuki fp2048_nayuki(2048);
thread_local FFT_Processor_nayuki fp4096_nayuki(4096);
...
EXPORT void IntPolynomial_ifft(LagrangeHalfCPolynomial* result, const IntPolynomial* p) {
    ...
    r->proc->execute_reverse_int(r->coefsC, p->coefs);
}
EXPORT void TorusPolynomial_ifft(LagrangeHalfCPolynomial* result, const TorusPolynomial* p) {
    LagrangeHalfCPolynomial_IMPL* r = (LagrangeHalfCPolynomial_IMPL*) result;
    r->proc->execute_reverse_torus32(r->coefsC, p->coefsT);
}
EXPORT void TorusPolynomial_fft(TorusPolynomial* result, const LagrangeHalfCPolynomial* p) {
    ...
    r->proc->execute_direct_torus32(result->coefsT, r->coefsC);
}
